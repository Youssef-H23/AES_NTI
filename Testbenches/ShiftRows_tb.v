...
module ShiftRows_tb;
    ...
    ShiftRows #(.STATE_WIDTH(STATE_WIDTH))
    dut
    (
        .state_in(state_in),
        .state_out(state_out)
    ...
    initial 
    begin
        $display("STARTING SHIFT ROWS TEST CASES");
        $display("--------------TEST CASE 1----------------");

        state_in = 128'h000102030405060708090a0b0c0d0e0f;
        expected = 128'h00050a0f04090e03080d02070c01060b;
        #5;
        ...
        expected = 128'h0055aaff4499ee3388dd2277cc1166bb;
        #5;
        if (state_out !== expected)
        begin
        ...
        expected = 128'h01abba1089dc3267fe5445ef7623cd98;
        #5;
        if (state_out !== expected)
        begin
        ...
        expected = 128'h1166bb0055aaff4499ee3388dd2277cc;
        #5;
        if (state_out !== expected)
        begin
        ...
        $display("--------------TEST CASE 5----------------");
        state_in = 128'hdeadbeef0123456789abcdef13579bdf;
        expected = 128'hde23cddf01ab9bef8957be6713ad45ef;
        #5;
        if (state_out !== expected)
        begin
        ...
        $display("----------------------------------------");
        $display("ALL SHIFT ROWS TEST CASES PASSED");
        $display("----------------------------------------");
        $stop;
    end
